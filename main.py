...
◇ الأسئلة الشائعة للطلاب الخريجين: <a href="https://dar.ksu.edu.sa/ar/gfaq">اضغط هنا</a>""",
    "تخصصات برامج الدبلوم - حضوري": {
        ...
        "options": [
            "دبلوم محاسبة", "دبلوم التسويق", "دبلوم الأنظمة", "دبلوم البرمجة وقواعد البيانات",
            "دبلوم السكرتارية الطبية", "دبلوم الاعمال المصرفية", "دبلوم الوسائط المتعددة التفاعلية",
            "دبلوم الامن السيبراني", "دبلوم السكرتارية التنفيذية", "دبلوم تقنية النظارات",
            "دبلوم إدارة الموارد البشرية", "دبلوم المحاسبة الضريبية", "دبلوم الترجمة بلغة الإشارة",
            "دبلوم مقدمي الرعاية بالحضانات", "دبلوم اللغة الصينية", "الدبلوم المشارك في العمل الخيري",
            "🔙 رجوع"
        ],
        "دبلوم محاسبة": {
            "menu_text": "الرجاء اختيار: ",
            "options": ["تعريف بالتخصص", "الخطة الدراسية", "وصف المقررات", "🔙 رجوع"],
...
""",
        ...
        "دبلوم الامن السيبراني": {
            "menu_text": "الرجاء اختيار: ",
            "options": ["تعريف بالتخصص", "الخطة الدراسية", "🔙 رجوع"],
...
""",
        ...
        "الدبلوم المشارك في العمل الخيري": {
            "menu_text": "الرجاء اختيار: ",
            "options": ["تعريف بالتخصص", "وصف المقررات", "🔙 رجوع"],
...
""",
    ...
    "تخصصات برامج الدبلوم - عن بُعد": {
        ...
        "options": [
            "شرح - البلاك بورد / Blackboard", "دبلوم الإدارة المالية والمصرفية - عن بعد.",
            "دبلوم إدارة أعمال التأمين - عن بعد.", "دبلوم التسويق – عن بعد",
            "خدمات طلابية ، متابعة الاعمال الفصلية", "كتب و ملخصات مواد الدبلوم عن بعد 📚 PDF",
            "🔙 رجوع"
        ],
...
""",       
    "كتب وملخصات مواد الدبلوم": {
        ...
        "options": ["📘1⃣- مستوى أول", "📘2⃣‐مستوى ثاني", "🔙 رجوع"],
        "📘1⃣- مستوى أول": {
...
# قائمة بالقنوات المطلوبة للاشتراك، مع معرّف القناة ورابطها.
REQUIRED_CHANNELS = [
...
# --- مُعالج أمر البدء (star) المحدث ---
@bot.message_handler(func=lambda m: m.text == "star")
def start_command_handler(message):
    chat_id = message.chat.id
    user_id = message.from_user.id

    # قائمة لتخزين القنوات التي لم يشترك بها المستخدم
    unsubscribed_channels = []

    # التحقق من اشتراك المستخدم في كل قناة من القنوات المطلوبة
    for channel in REQUIRED_CHANNELS:
        if not is_subscribed(user_id, channel['id']):
            unsubscribed_channels.append(channel)

    if unsubscribed_channels:
        # إذا كان هناك قنوات لم يشترك بها، يتم إرسال رسالة الاشتراك
        reply_message = "لاستخدام الروبوت يجب عليك أولاً الاشتراك في هذه الدردشات ⬇️"

        # إنشاء لوحة مفاتيح (Inline Keyboard) تحتوي على أزرار الاشتراك
        ...
        # إضافة رابط "منصة عون الأكاديمية" في رسالة منفصلة مع زر
...
def help_command_handler(message):
    ...
    bot.send_message(chat_id, reply, parse_mode="HTML", reply_markup=get_main_keyboard())
...
def handle_all_messages(message):
    ...
    if user_text == "🔙 رجوع":
        ...
        if user_text == "🤖 اسأل الذكاء الاصطناعي":
            ...
            # This case is now handled by the specific command handlers ("star", "help")
            # so we can leave a generic message for other unhandled text.
            pass # The default message will be sent
    elif current_state == "حضوري_sub_menu":
    ...
    # Check if a specific handler already sent a message
    if message.text in ["star", "help"]:
        return # The specific handlers already took care of the reply
